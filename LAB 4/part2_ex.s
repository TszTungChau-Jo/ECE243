...
CONFIG_PRIV_TIMER:                             
				...
				MOV      R2, #0x7                // enables interrupt and auto-refill of the timer; 0x7 = 0111; start counting
 ...
 ***************************************************************************************/
                .global  KEY_ISR
KEY_ISR:        
                /* Task 1. Toggle the RUN global variable */
				LDR      R0, RUN
				EOR      R0, #1                   // toggle by EORing with #1
				STR      R0, RUN
				
                /* Task 2. Clear the edge capture register values */
...
GO:				/* Case 1. RUN = 1, COUNT++, COUNT = 9, reset COUNT = 0 */
				LDR      R2, RUN
				CMP      R2, #1
				BNE      HOLD
				...
				LDR      R3, =0b1111111111        // check for max LED display 
				CMP      R0, R3   
				BGT      RESET                    // reset the counter when greater than #9
...
HOLD:			/* Case 2. RUN = 0, COUNT stays */
				LDR      R0, =0xFFFEC600          // base address of A9 Private Timer
				
				/* Task . Clear the edge capture register value */
				MOV      R1, #1
