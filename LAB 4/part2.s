...
/* Set up stack pointers for IRQ and SVC processor modes */
                  MOV      R1, #0b11010010     // interrupts masked to IRQ mode\
                  MSR      CPSR_c, R1         // switch to IRQ mode
                  LDR      SP, =0xFFFFFFFF - 3;

                  MOV      R1, #0b11010011     // interrupts masked to SVC mode
...
CONFIG_PRIV_TIMER:                             
                LDR      R0, =0xFFFEC600
                LDR      R1, =50000000        // set timer to create 0.25 interval
                STR      R1, [R0]
                MOV      R1, #0x7             // enable interrupt and automatic reload
                STR      R1, [R0, #0x8]
                MOV      PC, LR
...
CONFIG_KEYS:                                    
                LDR      R0, =0xFF200050
                MOV      R1, #0xF             // enable KEY[3:0] interrupt
                STR      R1, [R0, #0x8]
                MOV      PC, LR
...
IRQ_HANDLER:
                ...
                LDR      R4, =0xFFFEC100         // the base of GIC registers
                LDR      R5, [R4, #0x0C]         // read the interrupt ID from the interrupt acknowledge register
...
EXIT_IRQ:
                ...
                STR      R5, [R4, #0x10]          // store the ID of the interrupted I/O device back to the end-of-interrupt register
                POP      {R0-R7, LR}
                SUBS     PC, LR, #4
...
TIMER_ISR:
                ...
                LDR      R1, [R0, #0xC]
                MOV      R2, #1
                STR      R2, [R0, #0xC]       // clear edge capture
